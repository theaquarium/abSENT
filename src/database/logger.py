...
class Logger(metaclass=SingletonMeta):
    def __init__(self, path: str = "data/log.txt"):
        self.devMode = os.environ['absent_environment'] == 'dev'
        self.path = path
    ...
    def removedStudent(self, user: Student):
        self.log(
            f"STUDENT REMOVED \t| {user.first}; {user.last}; {user.number}")
    ...
    def editSchedule(self, user: Student, enumBlock: SchoolBlock, teacher: Teacher):
        self.log(
            f"USER SCHEDULE EDITED \t| {user.id}; {enumBlock}; {str(teacher)}")
    ...
    def adminAnnounce(self, number: str, message: str, school: SchoolName, grade: int):
        self.log(
            f"ADMIN ANNOUNCE \t| {number}; {message}; {str(school)}; {grade}")
    ...
    def canceledService(self, user: Student):
        self.log(
            f"SERVICE CANCELED \t| {user.id}; {user.first}; {user.last}; {user.number}")
...
class MessageLogger(metaclass=SingletonMeta):
    ...
    def log(self, sender_number: str, receiver_number: str, message: str):
        ...
        log_message = f"{current_time.strftime('%d/%m/%Y %H:%M:%S')} | {sender_number} \t| {receiver_number} \t| {message}\n"
